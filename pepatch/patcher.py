...
class Patcher(object):
    ...
    def _compile(self, *args, **kwargs):
        ...
        elif kwargs.has_key("nasm"):
            data = self.linker.assembler.nasm(kwargs["nasm"])
    ...
    def inject(self, *args, **kwargs):
        ...
        logger.success("Injected @ 0x%x" % (nextva))
        return nextva, len(data)

    def patch(self, va, *args, **kwargs):
        logger.info("Patching @ 0x%x" % (va))
        data = self._compile(base=va, *args, **kwargs)
        self.binary.writeva(va, data)
        return True

    def define(self, symbol, va):
        logger.info("Defined %s @ 0x%x" % (symbol, va))
